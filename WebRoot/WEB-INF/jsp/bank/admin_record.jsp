...
<html>
  <head>
	...
	<script>
	function topPage() {
		window.self.location = "manager.do?command=find_user_admin_Record&pageNo=${pagemodel.topPageNo}&pageSize=${managerForm.pageSize}"
	}
	function previousPage() {
		window.self.location = "manager.do?command=find_user_admin_Record&pageNo=${pagemodel.previousPageNo}&pageSize=${managerForm.pageSize}"
	}	
	
	function nextPage() {
		window.self.location = "manager.do?command=find_user_admin_Record&pageNo=${pagemodel.nextPageNo}&pageSize=${managerForm.pageSize}"
	}
	
	function bottomPage() {
		window.self.location = "manager.do?command=find_user_admin_Record&pageNo=${pagemodel.bottomPageNo}&pageSize=${managerForm.pageSize}"
	}
	</script>
  </head>
  ...
  <body>
  	<table width="98%" border="1" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px">
  		<tr bgcolor="#E7E7E7">
					<td height="14" colspan="7" align='center'>&nbsp;<bean:message key="the.administrator.modifies.the.record.sheet"/>&nbsp;</td>
		</tr>
  		<tr align="center" bgcolor="#FAFAF1" height="22">
  			<td width="23%">ID</td>
  			<td width="15%"><bean:message key="admin"/></td>
			<td width="15%"><bean:message key="type"/></td>
			<td width="15%"><bean:message key="user.username"/></td>
			<td width="15%"><bean:message key="user.password"/></td><br>
			<td width="15%"><bean:message key="inquiry"/></td><br>
			<td width="15%"><bean:message key="time"/></td><br>
  		</tr>
  	<c:choose>
  		<c:when test="${empty pagemodel.list}">
				<tr>
					<td colspan="7"><bean:message key="there.are.no.qualified.data"/></td>
				</tr>
				 </c:when>
  		<c:otherwise>
